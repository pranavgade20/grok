...
# beta - remove every other column and every other row from train set
...
from scripts.visualize_checkpoints import make_data
...
def train(hparams: Namespace) -> None:
    ...
    data = make_data(ArithmeticDataset, hparams.math_operator, hparams)

    train_dataset = data[::2,::2].reshape(-1,7)
    val_dataset = torch.cat([data[1::2].reshape(-1,7), data[::2,1::2].reshape(-1,7)])

    if hparams.batchsize == 0:
        hparams.batchsize = min(512, math.ceil(len(train_dataset) / 2.0))

    train_dataloader = DataLoader(
            train_dataset.to(device='cuda'),
            batch_size=hparams.batchsize,
            shuffle=True)

    val_dataset = val_dataset.to(device='cuda')
